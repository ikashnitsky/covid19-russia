...
p_arates <- df_calc %>%
    ...
    animation_slider(currentvalue = list(prefix = "Доля зараженного населения: "), font = list(color="E5E5E5FF")) %>%
    animation_button(hide = T) %>%
    hide_legend() %>% 
    layout(plot_bgcolor="#333333") %>% 
    layout(paper_bgcolor="#333333") 
...
p_arates_en <- df_calc %>%
    ...
    hide_colorbar() %>%
    animation_opts(1000, easing = "elastic", redraw = FALSE) %>%
    animation_slider(currentvalue = list(prefix = "Proportion of the population infected: "), font = list(color="E5E5E5FF")) %>%
    animation_button(hide = T) %>%
    hide_legend() %>% 
    layout(plot_bgcolor="#333333") %>% 
    layout(paper_bgcolor="#333333") 
...
p_vents <- df_vent %>%
    ...
    hide_colorbar() %>%
    animation_opts(1000, easing = "elastic", redraw = FALSE) %>%
    animation_slider(currentvalue = list(prefix = "Длительность эпидемии в неделях от начала до пика: "), font = list(color="E5E5E5FF")) %>%
    animation_button(hide = T) %>%
    hide_legend() %>% 
    layout(plot_bgcolor="#333333") %>% 
    layout(paper_bgcolor="#333333") 
...
p_vents_en <- df_vent %>%
    ...
    hide_colorbar() %>%
    animation_opts(1000, easing = "elastic", redraw = FALSE) %>%
    animation_slider(currentvalue = list(prefix = "Duration of the epidemic form the beginning to the peak, in weeks: "), font = list(color="E5E5E5FF")) %>%
    animation_button(hide = T) %>%
    hide_legend() %>% 
    layout(plot_bgcolor="#333333") %>% 
    layout(paper_bgcolor="#333333") 
